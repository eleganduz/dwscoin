...
#if defined(NDEBUG)
# error "Dwscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x98397ce77d4252536045fa3ee216a38035e0f96470613df298fdd40435a7691c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dwscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dwscoin Signed Message:\n";
...
{
    // Dwscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Dwscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Dwscoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Dwscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dwscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dwscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7e5d775c57e2335b3b28c4532d23967f427579d24751429c8e86794b5258f828");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "DWSCOIN UZBEKISTAN 18/10/2021 OOO 'UZ IT-TECHNOLOGY'";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c3bddc13df5f7e33a4c64cabd788ba8a02b9ff88f3e1ee9b1b52139c390e4e84662c413d1c0a95d678f69b287490c5cbc1b186fc8c6c6fcbae8f8a27f56e396c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1634501285;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089738142;

        if (fTestNet)
        {
            block.nTime    = 1634501272;
            block.nNonce   = 385571313;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x25babd50dd77926cdbd65b38c1cdf87ce807f90bb1e3b170d56918805902f640"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xdc }; // Dwscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DwscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DwscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DwscoinMiner(CWallet *pwallet)
{
    printf("DwscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dwscoin-miner");
    ...
    try { loop {
        ...
        printf("Running DwscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DwscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DwscoinMiner, pwallet));
}
